pipeline {
  ...
  environment {
    GITNAME = 'chohoon901'
    GITEMAIL = 'chohoon901@gmail.com'
    GITWEBADD = 'https://github.com/chohoon901/jenwp.git'
    GITSSHADD = 'git@github.com:chohoon901/jenwp.git'
    GITDEPADD = 'git@github.com:chohoon901/jenwp.git'
  ...
  stages {
    stage('Checkout Github') {
      steps {
        slackSend (channel: '#jenkins10', color: '#FFFF00', message: "STARTED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    ...
    stage('Docker image Build') {
      steps {
        ...
        sh "docker build -t ${DOCKERHUB2}:${currentBuild.number} -f db_Dockerfile ."
        sh "docker build -t ${DOCKERHUB2}:latest -f db_Dockerfile ."
    ...
    stage('k8s manifest file update') {
      steps {
        ...
        sh "sed -i 's@${DOCKERHUB2}:.*@${DOCKERHUB2}:${currentBuild.number}@g' deploy/deployment-wp.yml"
        sh "git add ."
